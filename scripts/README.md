...
# 障害福祉サービス (厚労省「介護給付費等単位数サービスコード」3 PDF を結合)
# 取得元: https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000174644_00021.html
for p in 1:620 001565790:269 001565791:562; do
  file=${p%:*}; pages=${p#*:}
  node extract_service_codes.mjs "../../../docs/介護ソフト関連/障害/障害サービスコード_$file.pdf" \
    "_tmp_$file.csv" 1 $pages --system=障害 --valid-from=2024-04-01 --notes=令和6年度報酬改定
done
# 結合 (inspect_csv.py の隣に merge スクリプトを置く想定 — 現状は手動)
...
## 障害福祉サービスカテゴリ一覧 (令和6年度報酬改定基準)

`service_codes_shogai.csv` (約 **101,578 件**) で抽出済み。代表的な prefix:
