...
function wrapFile(
  ...
  lines.push(
    [
      ...
      officeNumber,
      "0",
      // 媒体区分。⚠ 介護保険と障害福祉でコード体系が違う。
      //   介護保険 (共通編 P11): 1=伝送(ISDN) / 7=伝送(インターネット) → 現行は 7
      //   障害福祉 (_if_shogai.txt): **1=伝送** (インターネット請求もこれ)
      //   2026-07-17 に「全伝送ビルダーを 7 に統一」した際、障害まで 7 にしてしまった
      //   誤りを 2026-08-01 に是正。ほのぼのの障害伝送 (KJ/TJ/JJ) も実際に 1。
      "1", // 媒体区分 (障害は 1=伝送)
      processYm,
      "", // 予備 (設定しない)
    ].join(","),
...
export function buildShogaiDensou(
  ...
  for (const u of sorted) {
    ...
    // ⚠ 契約情報 (契約支給量・契約開始日・事業者記入欄番号) は**事業所ごとの契約**であって
    //   受給者証の支給決定量とは別物 (受給者証20時間を2事業所で分け合えば当事業所の契約は
    //   その一部)。ほのぼの実伝送 46 件のうち 12 件が受給者証の支給量と不一致だった。
    //   受給者証PDFの取込は contract_amount_text に受給者証の支給量を入れるだけで
    //   契約開始日は埋めないので、**契約開始日が空 = 契約情報が未入力**と判定できる。
    //   代替値のまま伝送すると誤った契約支給量・契約日を送ることになるため必ず通知する。
    if (codes.length > 0 && !u.contractStartDate) {
      warnings.push(
        `${r.user_name}: 契約情報が未入力のため受給者証の値で代替しています (契約支給量・契約開始日・事業者記入欄番号) — 受給者証ページの「契約支給量 (記入欄)」に当事業所の契約内容を転記してください。契約支給量は受給者証の支給量とは別です`,
      );
    }
    if (codes.length > 0) {
