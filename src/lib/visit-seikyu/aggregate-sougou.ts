...
const SOUGOU_PREFIX_BY_INSURER: Record<string, string> = {
  ...
  // 長生村 (長生郡)
  "124230": "CS_",
  // 大網白里市: 公式コード表(令和8年6月版 PDF)を専用prefix OA_ で投入済
  //   (import_sougou_oamishirasato.mjs)。独自A22系(訪問型サービスA/定率)も含む完全版。
  "122390": "OA_", // 大網白里市
  // ↓ 山武市/九十九里町は公式コード表が未提供のため暫定で茂原市(MB_)を流用中。
  //   6月実績はみなし現行相当(A21111=1176/A21211=2349)のみで、大網の本物伝送KK260702と
  //   単位数一致を確認済。各市の公式コード表が入ったら専用prefixに切替えること。
  "122374": "MB_", // 山武市 (暫定MB_流用)
  "124032": "MB_", // 九十九里町 (暫定MB_流用)
};
...
const SOUGOU_UNITPRICE_BY_INSURER: Record<string, number> = {
  "122101": 10.42, // 茂原市 (6級地・訪問系人件費70%) ← 実伝送で確定済 (級地単価式のアンカー)
  "124214": 10.0, // 一宮町 (その他地域)
  "124230": 10.0, // 長生村 (その他地域)
  // ↓ 令和6年度地域区分・訪問系人件費70%。単価 = 10 × (1 + 上乗せ率 × 0.70)。
  //   上乗せ率: 3級地15%→11.05 / 6級地6%→10.42 / 7級地3%→10.21 / その他0%→10.00。
  //   ★要照合: 大網の本物のほのぼの伝送(1275800892)が未入手のため級地は暫定。
  //   本物伝送との1円照合で確定すること (現状フォルダのKKは茂原の誤コピー)。
  "122390": 10.21, // 大網白里市 (7級地) ← 大網ほのぼの伝送KK260702で確定 (暫定6級地を是正)
  "122374": 10.21, // 山武市 (7級地) ← 同上で確定
  "124032": 10.0, // 九十九里町 (その他) ← 同上で確定
  // 千葉市 6区 (3級地)。従来は未登録で office.unit_price(茂原10.42)に誤フォールバックしていた
  "121012": 11.05, // 千葉市中央区
  "121020": 11.05, // 千葉市花見川区
  "121038": 11.05, // 千葉市稲毛区
  "121046": 11.05, // 千葉市若葉区
  "121053": 11.05, // 千葉市緑区
  "121061": 11.05, // 千葉市美浜区
