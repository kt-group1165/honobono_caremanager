...
export interface SupportRecordsContentProps {
  userId: string;
...
export function SupportRecordsContent({
  ...
  const [records, setRecords] = useState<SupportRecord[]>(initialRecords);
  ...
  // initial render は server からの initialRecords を使用、carePlan 切替時のみ refetch
  ...
  return (
    <>
      ...
      <div id="support-records-screen" className="flex-1 overflow-y-auto p-6">
      <div className="space-y-5">
        ...
        {!loadingRecords && showPreview && filteredRecords.length > 0 && selectedUser && (
          <div className="rounded-lg border-2 border-indigo-200 bg-white shadow-md">
            ...
            <div className="p-6 overflow-x-auto" style={{ fontFamily: '"MS Mincho","游明朝","Hiragino Mincho ProN",serif' }}>
              <PrintTable filteredRecords={filteredRecords} selectedUser={selectedUser} />
            </div>
          </div>
        )}
      ...
      <div id="support-records-print" className="hidden">
        {selectedUser && <PrintTable filteredRecords={filteredRecords} selectedUser={selectedUser} />}
      </div>
...
function PrintTable({
  ...
  filteredRecords: SupportRecord[];
  selectedUser: KaigoUser;
}) {
