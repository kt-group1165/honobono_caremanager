...
// ─── Config ────────────────────────────────────────────────────────────────
export const REPORT_CONFIG: Record<string, { titleJa: string; titleEn: string }> = {
...
export interface VisitCarePlanContent {
  // ヘッダー
...
export const emptyContent = (): VisitCarePlanContent => ({
...
export interface ReportsVisitContentProps {
  userId: string;
  reportType: string;
  initialContent: VisitCarePlanContent;
  initialDocId: string | null;
}

export function ReportsVisitContent({ userId, reportType, initialContent, initialDocId }: ReportsVisitContentProps) {
  const supabase = useMemo(() => createClient(), []);
  const config = REPORT_CONFIG[reportType];

  const [docId, setDocId] = useState<string | null>(initialDocId);
  const [content, setContent] = useState<VisitCarePlanContent>(initialContent);
