...
export function MeetingMinutesContent({
  ...
  // initial render は server からの initialDocs を使用、carePlan 切替時のみ refetch
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border bg-white shadow-sm p-4 space-y-3">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div>
            ...
            <input type="date" value={content.meeting_date} onChange={(e) => setField("meeting_date", e.target.value)} className={inp} />
          </div>
          <div>
            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">開催場所</label>
            <input type="text" value={content.location} onChange={(e) => setField("location", e.target.value)} className={inp} />
          </div>
          <div>
            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">開催時間</label>
            <input type="text" value={content.time_range} onChange={(e) => setField("time_range", e.target.value)} placeholder="14:00〜15:30" className={inp} />
          </div>
          <div>
            <label className="block text-[11px] font-medium text-gray-600 mb-0.5">開催回数</label>
            <input type="text" value={content.session_number} onChange={(e) => setField("session_number", e.target.value)} placeholder="第○回" className={inp} />
          </div>
        </div>
          ...
          <div className="grid grid-cols-[140px_1fr] gap-2">
            <div className="rounded border border-red-200 bg-red-50/30 p-2 text-[11px]">
              ...
              <label className="flex items-center gap-1.5">
                <input type="checkbox" checked={content.self_attended} onChange={(e) => setField("self_attended", e.target.checked)} className="h-3 w-3 accent-red-600" />
                <span>本人</span>
              </label>
              <label className="flex items-center gap-1.5 mt-1">
                <input type="checkbox" checked={content.family_attended} onChange={(e) => setField("family_attended", e.target.checked)} className="h-3 w-3 accent-red-600" />
                <span>家族</span>
              </label>
              <div className="mt-1">
                <input type="text" value={content.family_relationship} onChange={(e) => setField("family_relationship", e.target.value)} placeholder="続柄" className="w-full border border-gray-300 rounded px-1 py-0.5 text-[11px]" />
              </div>
              <div className="mt-2 border-t border-red-200 pt-1">
                <div className="text-[10px] text-gray-600 mb-0.5">※備考</div>
                <textarea rows={2} value={content.remarks} onChange={(e) => setField("remarks", e.target.value)} className="w-full border border-gray-300 rounded px-1 py-0.5 text-[11px] resize-none" />
              </div>
            </div>
        ...
        <div>
          <label className="block text-[11px] font-semibold text-gray-700 mb-0.5">検討内容</label>
          <textarea rows={4} value={content.discussion} onChange={(e) => setField("discussion", e.target.value)} className={ta} />
        </div>

        <div>
          <label className="block text-[11px] font-semibold text-gray-700 mb-0.5">結論</label>
          <textarea rows={3} value={content.conclusion} onChange={(e) => setField("conclusion", e.target.value)} className={ta} />
        </div>

        <div>
          <label className="block text-[11px] font-semibold text-gray-700 mb-0.5">残された課題（次回の開催時期）</label>
          <textarea rows={2} value={content.remaining_issues} onChange={(e) => setField("remaining_issues", e.target.value)} className={ta} />
        </div>
      </div>
      ...
      <div className="rounded-lg border-2 border-indigo-200 bg-white shadow-md">
        <div className="flex items-center justify-between border-b border-indigo-100 bg-indigo-50 px-4 py-2">
          ...
          <button type="button" onClick={onPrint} className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:underline">
            <Printer size={11} />
            印刷
          </button>
        </div>
  ...
  return (
    ...
    >
      <div style={{ position: "relative", marginBottom: "6px" }}>
        <div style={{ border: B, display: "inline-block", padding: "1px 8px", fontSize: "8pt" }}>第４表</div>
        <div style={{ position: "absolute", left: 0, right: 0, top: 0, textAlign: "center", fontSize: "13pt", fontWeight: "bold", letterSpacing: "0.2em" }}>サービス担当者会議の要点</div>
        <div style={{ position: "absolute", right: 0, top: "2px", fontSize: "8pt" }}>作成年月日　　年　　月　　日</div>
      </div>

      <div style={{ display: "flex", gap: "16px", fontSize: "9pt", marginBottom: "4px" }}>
        <div style={{ flex: 1, borderBottom: B }}>利用者名　{userName || "　　　　　　"}　殿</div>
      ...
      <div style={{ display: "flex", gap: "8px", fontSize: "9pt", marginBottom: "6px" }}>
        ...
        <div style={{ borderBottom: B, paddingRight: "8px" }}>開催時間　{content.time_range || "　　　　"}</div>
        <div style={{ borderBottom: B, paddingRight: "8px" }}>開催回数　{content.session_number || "　　"}</div>
      </div>
          ...
          <tr style={{ height: "18px" }}>
            <th rowSpan={4} style={{ ...thStyle, verticalAlign: "top", paddingTop: "4px" }}>
              ...
              <div style={{ marginTop: "6px", fontSize: "7pt", color: "#c00", textAlign: "left", lineHeight: 1.3 }}>
                利用者・家族の出席<br />
                本人：【{content.self_attended ? "✓" : "　"}】<br />
                家族：【{content.family_attended ? "✓" : "　"}】<br />
                （続柄：{content.family_relationship || "　　"}）<br /><br />
                ※備考
                <div style={{ marginTop: "2px", color: "#000", fontSize: "7pt", whiteSpace: "pre-wrap" }}>{content.remarks}</div>
              </div>
            </th>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "4px" }}>
        <tbody>
          <tr>
            ...
            <td style={{ ...cellBase, minHeight: "22px", whiteSpace: "pre-wrap" }}>{content.topics}</td>
          </tr>
          <tr>
            <td style={{ ...thStyle, fontWeight: "bold" }}>検討内容</td>
            <td style={{ ...cellBase, whiteSpace: "pre-wrap", height: "60px" }}>{content.discussion}</td>
          </tr>
          <tr>
            <td style={{ ...thStyle, fontWeight: "bold" }}>結論</td>
            <td style={{ ...cellBase, whiteSpace: "pre-wrap", height: "50px" }}>{content.conclusion}</td>
          </tr>
          <tr>
            <td style={{ ...thStyle, fontWeight: "bold", lineHeight: 1.2 }}>
              残された課題<br />
              <span style={{ fontSize: "7pt", fontWeight: "normal" }}>（次回の開催時期）</span>
            </td>
            <td style={{ ...cellBase, whiteSpace: "pre-wrap", height: "40px" }}>{content.remaining_issues}</td>
          </tr>
        </tbody>
      </table>
    </div>
