...
export function DisabilityContent({
  userId,
  initialRecords,
}: {
  userId: string;
  initialRecords: DisabilityCert[];
}) {
  const supabase = createClient();

  const [records, setRecords] = useState<DisabilityCert[]>(initialRecords);
  // 最新レコード自動選択 (lazy init)
  const [selectedId, setSelectedId] = useState<string | null>(
    initialRecords.length > 0 ? initialRecords[0].id : null,
  );
  const [form, setForm] = useState<FormData>(
    // eslint-disable-next-line react-hooks/immutability -- recordToForm は同 component 内 function 宣言 (hoist 済) 、lazy init で安全
