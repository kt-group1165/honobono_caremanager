/**
 * /users/[id] の詳細ペインの skeleton。
 *
 * サイドバーは親レイアウト (users/layout.tsx) で永続描画されるため、ここでは
 * 右の詳細ペインだけを skeleton 化する。これにより [id] 切替時にサイドバーは
 * 残り、右の詳細だけが一瞬 skeleton → 表示になる (全画面白フラッシュを解消)。
 *
 * 外側の padding / スクロール枠は users/layout.tsx が持つのでここでは付けない。
 */
