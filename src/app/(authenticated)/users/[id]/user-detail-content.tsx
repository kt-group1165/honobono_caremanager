...
function UserOfficeServices({
  ...
  // mutation 後の refetch のみで使う (mount-time fetch は server で実施済み)
  const load = async () => {
...
export function UserDetailContent({
  ...
  const [user, setUser] = useState<Client>(initialUser);
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);

  // 自事業所スコープのメモを currentOffice から派生
  const matchingMemo = useMemo(
    () => initialMemos.find((m) => m.scope === "tenant" && m.tenant_id === currentOffice?.tenant_id) ?? null,
    [initialMemos, currentOffice?.tenant_id],
  );
  const [tenantMemoId, setTenantMemoId] = useState<string | null>(matchingMemo?.id ?? null);

  const [form, setForm] = useState<EditForm>({
    name: initialUser.name ?? "",
    name_kana: initialUser.furigana ?? "",
    gender: initialUser.gender ?? "女",
    birth_date: initialUser.birth_date ?? "",
    blood_type: initialUser.blood_type ?? "",
    postal_code: initialUser.postal_code ?? "",
    address: initialUser.address ?? "",
    phone: initialUser.phone ?? "",
    mobile_phone: initialUser.mobile ?? "",
    email: initialUser.email ?? "",
    emergency_contact_name: initialUser.emergency_contact_name ?? "",
  ...
  // currentOffice が後から変わった場合 (テナント切替) は memo / form.notes を同期
  // initialMemos は server fetch 時点の全 memos なので、currentOffice 切替時は派生のみで十分
  ...
  useEffect(() => {
    ...
    setTenantMemoId(matchingMemo?.id ?? null);
    setForm((prev) => ({ ...prev, notes: matchingMemo?.body ?? "" }));
  }, [matchingMemo]);
