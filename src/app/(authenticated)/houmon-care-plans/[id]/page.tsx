...
import type { HoumonCarePlan } from "@/lib/houmon-care-plan/types";
    ...
    <div className="houmon-care-plan-print-root p-4 sm:p-6">
      ...
      <article className="mx-auto max-w-3xl bg-white p-6 print-page" style={{ minHeight: "auto" }}>
        <header className="mb-4 border-b-2 border-gray-700 pb-2">
          ...
          <p className="mt-1 text-xs text-gray-600">指定居宅サービス等の事業の人員、設備及び運営に関する基準 第28条</p>
        </header>

        {/* 利用者・事業所 */}
        <table className="w-full text-sm border border-gray-700 mb-3" style={{ borderCollapse: "collapse" }}>
          <tbody>
            <tr>
              <th className="border border-gray-700 bg-gray-100 px-2 py-1 text-left w-1/4">利用者氏名</th>
              <td className="border border-gray-700 px-2 py-1">
                {user?.name ?? "—"}
                {user?.furigana ? <span className="text-xs text-gray-500 ml-2">({user.furigana})</span> : null}
              </td>
              <th className="border border-gray-700 bg-gray-100 px-2 py-1 text-left w-1/6">要介護度</th>
            ...
            <tr>
              ...
              <td className="border border-gray-700 px-2 py-1">{formatJpDate(plan.plan_date)}</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <th className="border border-gray-700 bg-gray-100 px-2 py-1 text-left">サービス提供責任者</th>
              <td className="border border-gray-700 px-2 py-1">{plan.author_name || "—"}</td>
            </tr>
            <tr>
              <th className="border border-gray-700 bg-gray-100 px-2 py-1 text-left">有効期間</th>
              <td className="border border-gray-700 px-2 py-1" colSpan={3}>
        ...
        {/* 目標 */}
        <Block title="長期目標">{plan.long_term_goal || "—"}</Block>
        <Block title="短期目標">{plan.short_term_goal || "—"}</Block>
        ...
        {/* サービス内容 */}
        <div className="mb-3">
          <h2 className="text-sm font-semibold bg-gray-100 border border-gray-700 px-2 py-1">サービス内容</h2>
          <table className="w-full text-sm border border-gray-700" style={{ borderCollapse: "collapse" }}>
            <thead>
              <tr>
                ...
                <th className="border border-gray-700 bg-gray-50 px-2 py-1 w-1/6">頻度</th>
                <th className="border border-gray-700 bg-gray-50 px-2 py-1 w-1/6">時間帯</th>
                <th className="border border-gray-700 bg-gray-50 px-2 py-1">具体的内容</th>
              </tr>
            </thead>
            <tbody>
              {plan.services.length === 0 ? (
                ...
                plan.services.map((s, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="border border-gray-700 px-2 py-1">{s.kind || "—"}</td>
                    <td className="border border-gray-700 px-2 py-1">{s.frequency || "—"}</td>
                    <td className="border border-gray-700 px-2 py-1">{s.time_range || "—"}</td>
                    <td className="border border-gray-700 px-2 py-1 whitespace-pre-wrap">{s.content || "—"}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="mt-6 border-t pt-3">
          <table className="w-full text-sm border border-gray-700" style={{ borderCollapse: "collapse" }}>
            <tbody>
              <tr>
                <th className="border border-gray-700 bg-gray-100 px-2 py-1 text-left w-1/4">利用者同意日</th>
                <td className="border border-gray-700 px-2 py-1">{formatJpDate(plan.user_consent_date)}</td>
                <th className="border border-gray-700 bg-gray-100 px-2 py-1 text-left w-1/4">同意者署名</th>
                <td className="border border-gray-700 px-2 py-1">
...
function Block({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-3">
      <h2 className="text-sm font-semibold bg-gray-100 border border-gray-700 border-b-0 px-2 py-1">{title}</h2>
      <div className="border border-gray-700 px-2 py-2 text-sm whitespace-pre-wrap min-h-[3rem]">{children}</div>
    </div>
  );
}
