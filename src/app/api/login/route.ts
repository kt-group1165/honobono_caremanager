...
// POST /api/login
//
// Phase 11c trust-only: trusted_devices による端末信頼チェック (calendar-app 移植)
//
// 仕様:
//   1. login_id (または実 email) + password + device_id を受ける
//   2. signInWithPassword で認証 (server client が cookies に session を書く)
//   3. master user 以外は trusted_devices を引いて trust 判定:
//      - 行なし → pending 行作成 + session 即 signOut → 202 (approval_required)
//      - status=revoked → session 即 signOut → 403 (device_revoked)
//      - status=pending → last_seen_at 更新 + signOut → 202 (approval_required)
//      - status=approved → last_seen_at 更新 + session 維持 → 200 (ok)
//
// 注: kaigo-app には passkey 認証 endpoint が無いため、calendar-app の
//     passkey 排他チェック (hasPasskey && !emergencyAllowed) は移植しない。
//     trust check のみ。
...
export async function POST(request: Request) {
  ...
  // Phase 11c: device_id 必須化 (= trust check のため)
  ...
  // Phase 11c: trusted_devices check (PW ログイン経路にも適用)
  ...
  if (targetUser && !isMasterUser(targetUser.email)) {
    ...
    if (!trustRow) {
      ...
      const { error: insErr } = await admin
        .from("trusted_devices")
        .insert({ ...devicePayload, app: "kaigo" });
      if (insErr) {
