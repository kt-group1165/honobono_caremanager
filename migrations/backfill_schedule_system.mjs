...
`);
  ...
  console.log(`
  ⚠ 保留分 (介護取込なのに障害が付いている) は **伝送から起こした値**。
    2026-09-01 に 87 件を伝送で確認し、**障害のままで正しい**と決着した。
    対象 5 名の受給者証番号が 障害の KJ/TJ にすべて実在する
    (友寄 大 / 吉田 千里 / 秋山 久子 / 髙浦 末次 / 井口 恵子)。
    ほのぼのは稼働を介護エントリに入れつつ障害で請求することがあるので、
    **取込元のエントリは制度を決めない**。取込元に戻してはいけない。`);
