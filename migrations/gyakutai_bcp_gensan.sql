-- ============================================================================
-- 訪問介護 高齢者虐待防止措置未実施減算 / 業務継続計画(BCP)未策定減算 の適用期間管理
-- ============================================================================
-- 2026-07-11。訪問介護 (介護 system / サービス種類 11) のこの 2 減算 (各 1%) は
-- 独立減算コードが無く、基本コードの合成バリエーション (「身体７・虐防・Ⅰ」等) で
-- 算定する。事業所単位の「未実施/未策定」フラグを適用期間付き (改善までの期間性)
-- で持ち、集計 (src/lib/visit-seikyu/aggregate.ts) が対象月でこのテーブルを引いて
-- 基本サービス名を差し替える。テーブル未適用の環境は従来動作 (減算なし) に
-- フォールバックする (42P01/PGRST205 handling 済)。
...
COMMENT ON TABLE kaigo_office_gensan_periods IS
  '訪問介護 体制未整備減算 (gyakutai=高齢者虐待防止措置未実施 / bcp=業務継続計画未策定) の事業所別適用期間';
