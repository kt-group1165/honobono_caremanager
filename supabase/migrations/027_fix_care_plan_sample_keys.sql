-- 027_fix_care_plan_sample_keys.sql
-- 026 で投入した計画書サンプルが、フォームが期待する key と異なる key を
-- 使っていたため、画面で空欄に見えていた。
-- 正しい key で content を上書きする。
--
-- 第1表 expected keys:
--   plan_type, cert_status, creator_name, office_name,
--   user_name, birth_date, address,
--   care_level, cert_period, creation_date, initial_creation_date,
--   issue_analysis, review_opinion, overall_policy, living_support_reason
--
-- 第2表 expected keys:
--   user_name, creation_date,
--   needs_blocks: [{ needs, long_term_goal, long_term_period,
--                    goals: [{ short_term_goal, short_term_period, services: [...] }] }]
--
-- 第3表 expected keys:
--   user_name, creation_date, schedule (h00..h22 keys),
--   daily_activities, other_services
--   ※ 026 で正しく入っているはず
...
SET content = jsonb_build_object(
  ...
  'cert_period', c.start_date::text || '〜' || c.end_date::text,
...
FROM kaigo_users u, kaigo_care_certifications c
WHERE d.user_id = u.id
  AND d.certification_id = c.id
  AND d.report_type = 'care-plan-1';

-- ───────────────────────────────────────────────────────────────────────────
-- 第2表 上書き（needs_blocks 構造を正しい形に修正）
-- ───────────────────────────────────────────────────────────────────────────
UPDATE kaigo_report_documents d
SET content = jsonb_build_object(
  'user_name', u.name,
  ...
  'needs_blocks', jsonb_build_array(
    jsonb_build_object(
      ...
      'long_term_period', c.start_date::text || '〜' || c.end_date::text,
      'goals', jsonb_build_array(
        jsonb_build_object(
          'short_term_goal', '下肢筋力を維持し、転倒なく歩行できる',
          'short_term_period',
            c.start_date::text || '〜' || (c.start_date + INTERVAL '6 months')::date::text,
          'services', jsonb_build_array(
            jsonb_build_object(
              ...
              'frequency', '週2回',
              'period', c.start_date::text || '〜' || c.end_date::text
            ),
            jsonb_build_object(
              'content', '歩行器の貸与',
              'insurance_flag', '○',
...
-- ───────────────────────────────────────────────────────────────────────────
-- 第3表 上書き（schedule key をフォーム期待値 h00..h22 に合わせて確認）
-- 026 でも h08/h10/h14/h16 のキーを使っているので問題なし
-- ただし user_name と creation_date を確実に入れる
-- ───────────────────────────────────────────────────────────────────────────
UPDATE kaigo_report_documents d
SET content = jsonb_build_object(
  'user_name', u.name,
  'creation_date', (CURRENT_DATE - INTERVAL '7 days')::text,
  'care_level', c.care_level,
  'plan_period', c.start_date::text || '〜' || c.end_date::text,
