-- 022_assessment_form_data.sql
-- 現在のアセスメントフォームは form_data (JSONB) に全データを保存している。
-- 018-021 で挿入したサンプルは旧個別カラム (family_situation 等) にしか
-- 入っていなかったため、フォームを開いても空欄に見えていた。
--
-- このマイグレーションは サンプルアセスメント
-- (assessor_name = 'ケアマネジャー（サンプル）') の form_data に
-- リッチな JSON を投入する。
